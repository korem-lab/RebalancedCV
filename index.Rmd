...
Welcome to [_RebalancedCV_](index.html)! This is a python package designed to facilitate correcting for distributional bias during cross validation.  

<br>
&nbsp;
</div>
RebalancedCV conatins the following three classes: [_RebalancedLeaveOneOut_](RebalancedLeaveOneOut.html), [_RebalancedKFold_](RebalancedKFold.html), and [_RebalancedLeavePOut_](RebalancedLeavePOut.html). These are implementations for `Leave One Out`, `Stratified K-Fold`, and `Leave P Out` cross-validation with sub-sampling within training sets to ensure that, for any cross-validation scheme, the class balances across all training folds are identical.
...
The only dependencies for installation are `python` and `scikit-learn`
<br><br>
...
As demonstrated in this example, neglecting to account for distributional bias in the cross-validation classes can greatly decrease evaluated model performance. For more details on why this happens, please refer to [our manuscript](google.com).
...
Please report any issues faced while using RebalancedCV in our [Issues Page](https://github.com/korem-lab/RebalancedCV/issues), or email [gia2105@columbia.edu](gia2105@columbia.edu). 
