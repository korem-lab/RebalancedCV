...
RebalancedCV contains the following three classes: [__RebalancedLeaveOneOut__](RebalancedLeaveOneOut.html), [__RebalancedKFold__](RebalancedKFold.html), and [__RebalancedLeavePOut__](RebalancedLeavePOut.html). These are implementations for `Leave One Out`, `Stratified K-Fold`, and `Leave P Out` cross-validation with sub-sampling within training sets to ensure that, for any cross-validation scheme, the class balances across all training folds are identical.
...
```python
from rebalancedcv import RebalancedLeaveOneOut, RebalancedKFold, RebalancedLeavePOut
```
<br>
