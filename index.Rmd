...
RebalancedCV contains the following classes: [__RebalancedLeaveOneOut__](RebalancedLeaveOneOut.html), [__RebalancedKFold__](RebalancedKFold.html), [__RebalancedLeavePOut__](RebalancedLeavePOut.html), [__RebalancedLeaveOneGroupOut__](RebalancedLeaveOneGroupOut.html), [__RebalancedLeaveOneOutRegression__](RebalancedLeaveOneOutRegression.html), and [__MulticlassRebalancedLeaveOneOut__](MulticlassRebalancedLeaveOneOut.html). These are implementations for `Leave One Out`, `Stratified K-Fold`, `Leave P Out`, and `Leave One Group Out` cross-validation with sub-sampling within training sets to ensure that, for any cross-validation scheme, the class balances across all training folds are identical.
...
```python
from rebalancedcv import RebalancedLeaveOneOut, RebalancedKFold, RebalancedLeavePOut, RebalancedLeaveOneGroupOut, RebalancedLeaveOneOutRegression, MulticlassRebalancedLeaveOneOut
```
<br>
...
We note that the example's code structure approach would apply to this package's other `RebalancedKFold`, `RebalancedLeavePOut`, `RebalancedLeaveOneGroupOut`, and `RebalancedLeaveOneOutRegression` classes.



All classes from this package provide train/test indices to split data in train/test sets while rebalancing the training set to account for distributional bias. This package is designed to enable automated rebalancing for the cross-validation implementations in scikit-learn's `LeaveOneOut`, `StratifiedKFold`, `LeavePOut`, and `LeaveOneGroupOut`, through the `RebalancedCV` classes `RebalancedLeaveOneOut`, `RebalancedKFold`, `RebalancedLeavePOut`, `RebalancedLeaveOneGroupOut`, and `RebalancedLeaveOneOutRegression`. These Rebalanced classes are designed to work in the exact same code structure and implementation use cases as their scikit-learn equivalents, with the only difference being a subsampling within the provided training indices. 

For more details, we provide further background and walkthroughs for each class in their respective documentation tabs.
...
<div class="citation">
Austin, G.I., Pe'er I., & Korem T. “Distributional bias compromises leave-one-out cross-validation” Science Advances 11(48): eadx6976 (2025). [10.1126/sciadv.adx6976](https://doi.org/10.1126/sciadv.adx6976)
</div>
